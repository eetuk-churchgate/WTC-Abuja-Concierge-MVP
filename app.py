...
def save_lead(d):
    ...
    sql = f"""INSERT INTO leads(id,first_name,last_name,email,phone,company,job_title,timing,materials,tags,inspection,marketing,campaign,device_id,submitted,source,quality)
    VALUES('{lid}','{fn}','{ln}','{em}','{ph}','{co}','{jt}','{ti}','{mats}','{tags}',{ins},{mk},'NOG Energy Week 2026','{did}','{now}','{src}','{qual}')"""
...
def export_csv():
    ...
    if not rows: return ""
    o = io.StringIO()
    w = csv.DictWriter(o,fieldnames=['submitted','first_name','last_name','email','phone','company','job_title','materials','inspection','marketing','source','quality'],extrasaction='ignore')
    w.writeheader()
    for r in rows:
...
ROUTES={
    "overview":{"title":"Overview","screens":[
        ...
        {"t":"The Proof","b":"A working, operational building — not a promise.","h":[("33,180 m²","Office GLA"),("1,440 m²","Typical Floorplate"),("120","Executive Residences"),("500+","CCTV Cameras"),("CBD Address","Near NNPC & Petroleum Ministry")]}]},
    ...
    "video":{"title":"Video Walkthrough","screens":[
        {"t":"Experience WTC Abuja","b":"Watch a premium walkthrough of the completed development — offices, residences, clubhouse, security, and surrounding infrastructure.","video":True},
        {"t":"360° Virtual Tour","b":"Explore WTC Abuja interactively. Use your finger to pan and explore every angle of the development.","tour":True}]}
}
    ...
    if "interactive" in s and s["interactive"]:
        ...
        for i, fp in enumerate(fps):
            with cols[i]:
                ...
                st.markdown(f'<div style="background:{bg};border:{border};border-radius:6px;padding:18px 10px;text-align:center;margin:3px;cursor:pointer;min-height:120px" onclick=""><div style="font-size:1.3rem;color:{fp["color"]};font-weight:700">{fp["size"]}</div><div style="font-size:0.7rem;color:#b8b4ac;font-family:Arial,sans-serif;margin-top:4px">{fp["use"]}</div></div>',True)
    ...
    with c3:
        if st.button("Submit →",key="cvsb",use_container_width=True,type="primary"):
            if not fn or not ln or not em or not ph or not co: st.error("Fill all required fields")
                ...
                ld={"fn":fn,"ln":ln,"em":em,"ph":ph,"co":co,"jt":jt,"ti":ti,"mt":selected,"tg":list(set(tm.get(m,m) for m in selected)),"ins":"Request a Private Inspection" in selected,"mk":mk,"src":st.session_state.get("source","direct"),"qual":{}}
                save_lead(ld)
                send_lead_email(ld)
